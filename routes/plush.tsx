...
export const handler: Handlers<MetaProperties> = {
    async GET(_req, ctx) {
        const args: DatabaseProps = {
            column: "url",
            table: "plush",
            identification: "plush"
        }
        const metaProperties = await MetaUrlPreview(args);
        metaProperties["redirect_url"] = "https://www.makeship.com/products/anny-gitd-plush"
        return ctx.render(metaProperties);
    },
};

export default function Plush(props: PageProps<MetaProperties>) {
    return (
        <div>
            <Head>
